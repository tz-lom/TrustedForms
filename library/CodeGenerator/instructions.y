...
validation_rule(cmd) ::= IDENTIFIER(id) EQUALS SQARE_LBRACKET param_list(list) SQARE_RBRACKET.	{ cmd = new Check(id->value,list); }

vr_ct_class_name(c) ::= IDENTIFIER(v).	{ c = v->value; }
vr_ct_class_name(c) ::= STRING(v).		{ c = v->value; }

vr_ct_rule(t) ::= PLUS  vr_ct_class_name(c).	{ t = array('className'=>'AddClass'	, 'class'=>c); }
vr_ct_rule(t) ::= MINUS vr_ct_class_name(c).	{ t = array('className'=>'RemoveClass', 'class'=>c); }
...
vr_definition(def) ::= CSS(css) HTML(html).						{ def = array(DisplayMessage::instance(css->value)->setText(html->value)); }
...
rules_list(set) ::= validator(rule).						{ set = Rules::instance()->addRule(rule); }
rules_list(set) ::= rules_list(old) COMA validator(rule).	{ set = old->addRule(rule); }

rules(rules) ::= CURLY_LBRACKET rules_list(list) CCURLY_RBRACKET.         { rules = list; }

element_rules_definition(def) ::= CSS(css) rules(r).	{ def = new Field(css,r); }
